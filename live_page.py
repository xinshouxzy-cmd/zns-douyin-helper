...
class LiveMonitor(QThread):
    ...
    def __init__(self, cfg, parent=None, window_handle=None):
        ...
        self._fg_before_trigger = None  # 触发前的前台窗口（特效完恢复用）
    ...
    def _schedule_return_main(self, delay, codes, hold=0.15):
        ...
        def _do():
            ...
            if self._run and self._return_seq == my_seq:
                try:
                    send_hotkey(codes, hold=hold, repeat=3)
    ...
    def _restore_foreground(self):
        """恢复触发前的前台窗口（特效结束回到主镜头后调用）"""
        hwnd = self._fg_before_trigger
        self._fg_before_trigger = None
        if hwnd:
    ...
    def _monitor(self):
        ...
        fg_switch = bool(cfg.get("foreground_trigger", True))  # 触发时把直播伴侣置顶（特效完恢复）
        scenes = []
                    ...
                    for item in scenes:
                        ...
                        if any(kw in match_text for kw in ks):
                            if now - last_t >= cooldown:
                                item[2] = now
                                # 立即作废上一场景的"返回主镜头"计时（必须先于发送热键，
                                # 否则保持时间短时旧计时会在新场景热键发完前触发，误回主镜头）
                                self._return_seq += 1
                                # 热键在监控线程里直接发送（不等 UI 线程排队），
                                # 发送参数与测试按钮完全一致（按住 0.25s + 连发 3 遍）
                                self.log.emit(f"[green]⚡ 命中场景「{ks[0]}」→ 发送热键 {hk}（连发3遍，按住{max(key_hold, 0.25):g}s）")
                                if sys.platform == "win32":
                                ...
                                if main_codes and sys.platform == "win32" and hold_seconds > 0:
                                    ...
                                    # 未配置自动回主镜头：5 秒后恢复原前台窗口（兜底，避免一直占前台）
                                    threading.Thread(
                                        target=lambda: (time.sleep(5), self._restore_foreground()),
                                        daemon=True).start()
                            break
...
class LivePage(QWidget):
    ...
    def _load_to_ui(self):
        ...
        self.chk_foreground.setChecked(bool(self.cfg.get("foreground_trigger", True)))
    ...
    def _collect(self):
        ...
        return {
            ...
            "foreground_trigger": self.chk_foreground.isChecked(),
            "scenes": scenes,
            "knowledge": knowledge,
        }
